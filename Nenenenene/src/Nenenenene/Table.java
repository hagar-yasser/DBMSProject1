...
//				Page old = deserialize(Name, index);
        ...
        for (String k : keys) {
        	for(int i=0;i<BPTrees.size();i++) {
        		if(BPTrees.get(i).ColName.equals(k)) {
        			BPTrees.get(i).insert((Comparable)HtblColNameValue.get(k), recordrefrence);
    ...
    public void deleteFromIndex(Hashtable<String, Object> HtblColNameValue,Ref recordrefrence) throws DBAppException {
        ...
        for (String k : keys) {
        	for(int i=0;i<BPTrees.size();i++) {
        		if(BPTrees.get(i).ColName.equals(k)) {
        			BPTrees.get(i).deleteSingleRef((Comparable)HtblColNameValue.get(k), recordrefrence);
                ...
                for (String k : keys) {
                	for(int j=0;j<BPTrees.size();j++) {
                		if(BPTrees.get(j).ColName.equals(k)) {
                			BPTrees.get(j).updateRef((Comparable)current.Rows.get(i).row.get(k), old, neww);
                		}
                	}
                }
